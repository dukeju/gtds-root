    	...
    	<fieldset>
    		...
    		<table width="100%" border="1" cellspacing="0">
    			...
    			<s:if test="#request.selectedTask != null">
	    			<tr>
	    				<td>${requestScope.selectedTask.task.name}</td>
	    				<td>${requestScope.selectedTask.tutor.name }</td>
	    				<td><s:property value="%{#request.selectedTask.pass? '通过' : '未通过'}" /></td>
	    				<td>${requestScope.selectedTask.task.capacity }</td>
	    				<s:set value="#request.selectedTask.task.id" var="taskId"></s:set>
	    				<td><s:property value="getSelectedCount(#taskId)" /></td>
	    				<td><s:a action="TaskAction_showTaskInfo?taskId=%{#taskId}">详情</s:a></td>
	    				<td>
	    					<s:set value="#request.selectedTask.task.student.id" var="sId"></s:set>
    	 					<s:if test="#sId == null">
		    					<a href="" id="unselect">退选</a>
	    					</s:if>
	    					<s:else>
		    					<a href="" id="unselect">取消自拟</a>
	    					</s:else>
	    					<s:hidden value="%{#request.selectedTask.task.id}"></s:hidden>
	    				</td>
	    			</tr>
    			</s:if>
    		</table>
